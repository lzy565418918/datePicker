...
export default {
  ...
  //方法集合
  methods: {
    // 针对日选择器出现的2月份对应天数不正确的处理
    changeYear(columnIndex, itemIndex, value) {
      // 获取到所选年月份对应的天数
      let obj = this.$refs[`picker0`].getColumnValues();
      // 当所选年不为“全部”时才处理
      if (value.value != "all") {
        // 给第2列（天数所在列）赋值，并刷新天数所在列
        this.$refs[`picker0`].setColumnValues(2, obj[1].children, vm => {
          vm.refresh(null, 2);
        });
      }
    },
    ...
    hide(index) {
      if (!this[`confirm${index}`]) {
        this.$refs[`picker${index}`].refresh(null, 0);
      }
    },
    // 选好日期后点击确定，筛选数据
    onPickerConfirm(index) {
      ...
      console.log(
        `[Mand Mobile] Picker getColumnValues: ${value.substr(
          0,
          value.length - 1
        )}`
      );
    ...
    getDatetime(dataTime) {
      let month =
        dataTime.getMonth() + 1 < 10
          ? String("0" + (dataTime.getMonth() + 1))
          : String(dataTime.getMonth() + 1);
      let date =
        dataTime.getDate() < 10
          ? String("0" + dataTime.getDate())
          ...
          for (let p = 0; p < monthlen; p++) {
            ...
            if (
              ...
              for (let j = 0; j < new Date().getDate(); j++) {
                ...
                dateNum =
                  dateNum < 10 ? String("0" + dateNum) : String(dateNum);
